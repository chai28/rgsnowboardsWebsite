<?php
    ...
    try{
        $stmt = $conn->prepare("SELECT blogName, blogContent, thumbnail, blogDate, blogCatID FROM blog WHERE blogId = :blogId");
        ...
        $stmt = $conn->prepare("SELECT * FROM blog_carousel WHERE blogId = :blogId");
        $stmt->execute(['blogId'=>$blogId]);
        $row3 = $stmt->fetch();
...
<div class="row">
    ...
    <div class="card-body p-0">
        <!-- Post Form -->
        <?php echo "<form action='editPost.php?id=$blogId' method='POST' enctype='multipart/form-data'>";?>
            ...
            <div class="form-group">
                ...
                <div><img  src="<?php echo $row1['thumbnail']; ?>" style="width: 200px; height: 140px; margin-left: 40px;"/></div>
                <br>

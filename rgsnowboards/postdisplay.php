<?php
	...
	if(isset($_POST['post'])){
		...
		try{
			...
			$stmt = $conn->prepare("INSERT INTO blog (blogName, thumbnail, blogContent, blogCatID, blogDate)
            values(:title,:thumbnail,:body,:categoryid,NOW());");
			$stmt->execute(['title'=>$title, 'thumbnail'=>$filename, 'body'=>$body, 'categoryid'=>$categoryid,]);
			...
			for( $i=0 ; $i < $total ; $i++ ) {
				...
				if ($tmpFilePath != ""){
					...
					if(move_uploaded_file($tmpFilePath, $newFilePath)) {
						...
						$stmt = $conn->prepare("INSERT INTO blog_carousel (blogId, image)
						values(:id,:filename);");
